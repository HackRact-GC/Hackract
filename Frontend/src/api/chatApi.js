...
export const startDirectConversation = (targetUserId) =>
  api
    .post(`${BASE}/conversations/direct`, { targetUserId })
    .then((r) => r.data.data.conversation);

export const createGroupConversation = (name, participantIds, avatar) =>
  api
    .post(`${BASE}/conversations/group`, { name, participantIds, avatar })
    .then((r) => r.data.data.conversation);
...
export const sendMessage = (conversationId, payload) =>
  api
    .post(`${BASE}/conversations/${conversationId}/messages`, payload)
    .then((r) => r.data.data.message);

export const editMessage = (conversationId, messageId, content) =>
  api
    .patch(`${BASE}/conversations/${conversationId}/messages/${messageId}`, {
      content,
    })
    .then((r) => r.data.data.message);
...
export const deleteMessage = (conversationId, messageId) =>
  api
    .delete(`${BASE}/conversations/${conversationId}/messages/${messageId}`)
    .then((r) => r.data.data);
...
export const searchUsers = (q, role) =>
  api
    .get(`${BASE}/users/search`, { params: { q, role } })
...
export const addParticipants = (conversationId, userIds) =>
  api
    .post(`${BASE}/conversations/${conversationId}/participants`, { userIds })
    .then((r) => r.data.data);

export const leaveConversation = (conversationId) =>
  api
    .delete(`${BASE}/conversations/${conversationId}/leave`)
    .then((r) => r.data.data);
...
export const sendInvitation = (targetUserId, message) =>
  api
    .post(`${BASE}/invitations`, { targetUserId, message })
    .then((r) => r.data.data);
