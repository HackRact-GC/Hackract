...
api.interceptors.request.use(
  (config) => {
    const reqConfig =
      /** @type {import('axios').InternalAxiosRequestConfig & { _isRefreshRequest?: boolean }} */ (
        config
      );
    // Skip adding the header for the refresh endpoint itself
    if (!reqConfig._isRefreshRequest) {
...
api.interceptors.response.use(
  ...
  async (error) => {
    const originalRequest =
      /** @type {import('axios').InternalAxiosRequestConfig & { _retry?: boolean }} */ (
        error.config
      );
    ...
    try {
      const refreshConfig =
        /** @type {import('axios').AxiosRequestConfig & { _isRefreshRequest?: boolean }} */ ({
          _isRefreshRequest: true,
        });
      const { data } = await api.post(
        "/auth/local/refresh",
        { refreshToken: storedRefreshToken },
        refreshConfig, // flag to skip the request interceptor
      );
