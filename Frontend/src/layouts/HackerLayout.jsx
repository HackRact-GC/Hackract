...
import { isOrgAdminMember, ROLES } from "../utils/roles.js";
const HackerLayout = () => {
  ...
  const isActive = (route) => {
    if (route === "/hacker-dashboard")
      return location.pathname === "/hacker-dashboard";
    return location.pathname.startsWith(route);
  };

  // Current page label for the header
  const currentPage =
    navItems.find((n) => isActive(n.route))?.label ?? "Hackract";
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-gray-300 font-sans selection:bg-[#00c477]/30 overflow-hidden">
      ...
      {/* ── SIDEBAR ───────────────────────────────────────────────────── */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#050505] border-r border-white/5 flex flex-col justify-between transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div>
          {/* Logo & mobile close */}
          ...
          <nav className="space-y-1 px-3">
            {navItems.map((item) => (
              ...
              >
                <item.icon
                  className={isActive(item.route) ? "text-[#00c477]" : ""}
                />
