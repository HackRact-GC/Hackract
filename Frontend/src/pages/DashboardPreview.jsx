...
const DashboardPreview = () => {
  ...
  const criticalVulns = findings.filter(
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto block w-full space-y-6 lg:space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          {[
            ...
            <div
              key={i}
              className="bg-[#111111] border border-white/5 rounded-2xl p-6 flex flex-col relative overflow-hidden"
            >
              {stat.isPrimary && (
                <div className="absolute top-0 right-0 w-24 h-24 bg-[#00ff88]/5 rounded-bl-full filter blur-xl"></div>
              )}
              ...
              <div className="flex items-end justify-between">
                <div className="text-4xl font-bold text-white flex items-center gap-2">
                  {stat.value}
                  {stat.alert && (
                    <span className="text-xs bg-red-500/10 text-red-500 px-2 py-1 rounded-md tracking-widest font-mono">
                      CRITICAL
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-0 mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Managing ongoing penetration tests and security audits
                </p>
              </div>
              <button
                onClick={() => navigate("/projects")}
                className="text-sm px-4 py-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-white transition-colors w-full sm:w-auto"
              >
                View All Projects
              </button>
            </div>
              ...
              {loading ? (
                ...
                activeOperativesProjects.map((p) => {
                  const pFindings = findings.filter(
                    (f) => f.pentestId === p.id || f.projectId === p.id,
                  );
                  const criticalCount = pFindings.filter(
                    (f) => f.severity === "CRITICAL",
                  ).length;
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`absolute top-6 right-6 text-xs font-mono font-bold px-2 py-0.5 rounded ${p.status === "IN_PROGRESS" ? "text-black bg-[#00ff88]" : p.status === "PLANNING" ? "text-[#b490ff] bg-[#b490ff]/10 border border-[#b490ff]/30" : "text-gray-400 bg-white/5"}`}
                      >
                        {p.status || "ACTIVE"}
                      </div>
                      <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                          ...
                          <span
                            className={`${criticalCount > 0 ? "text-red-400 font-bold" : pFindings.length > 0 ? "text-yellow-500 font-bold" : "text-gray-500"}`}
                          >
                            {pFindings.length} Found
                          </span>
                        </div>
          ...
          <div className="space-y-6 mt-8 lg:mt-0">
            ...
            <div className="bg-[#111111] border border-white/5 rounded-2xl p-6">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                  <h2
                    className="text-lg font-bold text-white cursor-pointer hover:text-[#00ff88] transition-colors"
                    onClick={() => navigate("/hacker-profile")}
                  >
                    {displayName}
                  </h2>
            ...
            <div className="bg-[#111111] border border-white/5 rounded-2xl p-5">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[10px] bg-[#00ff88]/20 text-[#00ff88] px-2 py-0.5 rounded font-bold">
                  LIVE
                </span>
              </div>
              ...
              <div className="space-y-3">
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm font-bold text-white">
                        Project Activity: {p.name}
                      </div>
                      <div className="text-xs text-[#00ff88] mt-0.5">
                        Recent updates detected in workspace.
                      </div>
                    </div>
                  </div>
                ))}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 truncate mt-0.5">
                      Check your inbox for communications.
                    </div>
                  </div>
                </div>
                ...
                {criticalVulns > 0 && (
                  <div
                    ...
                    className="flex items-start gap-3 p-3 bg-red-500/10 rounded-xl border-l-2 border-red-500 cursor-pointer"
                  >
                    ...
                    <div>
                      <div className="text-sm font-bold text-white">
                        Critical Alerts ({criticalVulns})
                      </div>
                      <div className="text-xs text-red-400 truncate mt-0.5">
                        Immediate attention required on findings.
                      </div>
                    </div>
                  </div>
                )}
              ...
              <button
                onClick={() => navigate("/findings")}
                className="w-full text-center text-xs text-gray-400 hover:text-white mt-4 font-bold tracking-widest"
              >
                VIEW ALL ALERTS
              </button>
            </div>
