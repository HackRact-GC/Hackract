  ...
  <div className="bg-[#050505] border border-white/5 p-8 rounded-[32px] flex flex-col justify-between h-48 relative overflow-hidden group hover:border-[#00c477]/20 transition-all duration-500 shadow-2xl">
    <div className="flex items-start justify-between relative z-10">
      <div className="space-y-1">
        <p className="text-[10px] font-mono font-black text-gray-600 uppercase tracking-[.2em]">
          {label}
        </p>
      ...
      <div
        className={`p-3 rounded-2xl bg-white/2 border border-white/5 ${color} group-hover:scale-110 transition-transform duration-500`}
      >
        <Icon size={24} />
      </div>
    </div>
    ...
    <div className="relative z-10 flex items-center justify-between">
      ...
      {avatars && (
        <div className="flex -space-x-2 mr-auto">
          {avatars.map((av, i) => (
            <div
              key={i}
              className="w-8 h-8 rounded-full border-2 border-[#050505] bg-gray-800 flex items-center justify-center text-[10px] font-black text-white overflow-hidden shadow-lg"
            >
      ...
      {trend && (
        <span className="text-[10px] font-mono font-black text-[#00c477] tracking-widest">
          {trend}
        </span>
      )}

      {sub && (
        <span className="text-[9px] font-mono font-black text-[#ff3366] uppercase tracking-widest leading-none text-right">
          {sub}
        </span>
      )}
    </div>
          ...
          <tbody className="divide-y divide-white/3">
            {displayProjects.map((p, i) => (
              <tr key={i} className="hover:bg-white/1 transition-all group">
                <td className="px-10 py-6">
                  <div>
                    <p className="text-sm font-black text-white uppercase tracking-tight group-hover:text-[#00c477] transition-colors">
                      {p.name}
                    </p>
                ...
                <td className="px-10 py-6">
                  <span
                    className={`text-[8px] font-black px-2.5 py-1 rounded-md border tracking-widest ${
                ...
                <td className="px-10 py-6 text-center">
                  <span className="text-xs font-mono font-bold text-gray-400">
                    {p.collaborators?.find((c) => c.role === "PROJECT_ADMIN")
                      ?.user?.fullName || "N/A"}
                  </span>
                </td>
            ...
            {displayProjects.length === 0 && (
              <tr>
                <td
                  colSpan={4}
                  className="px-10 py-20 text-center text-[10px] text-white/20 uppercase tracking-widest font-mono"
                >
...
const formatAction = (log) => {
  ...
  const projectName =
    pentest?.name || details?.projectName || details?.name || "Unknown Project";
    ...
    <div className="bg-[#050505] border border-white/5 rounded-[32px] p-10 flex flex-col h-full shadow-2xl relative overflow-hidden group">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-[#00c477]/10 text-[#00c477] rounded-lg">
            <FiActivity />
          </div>
          <h3 className="text-sm font-black text-white tracking-widest uppercase font-mono">
            Recent Activity
            ...
            {activities.map((log) => {
              if (log.type === "finding") {
                ...
                return (
                  <div
                    key={log.id}
                    className="relative pl-6 animate-in fade-in slide-in-from-bottom-2 duration-300"
                  >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1 flex-1">
                        <div className="text-xs text-gray-300 leading-tight">
                          ...
                          reported a new{" "}
                          <span
                            className={`font-black uppercase tracking-wider text-[10px] ${getSeverityColor(severity)}`}
                          >
                            {severity}
                          </span>{" "}
                          finding:{" "}
...
const OrganizationDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const merged = [...activities, ...findings].sort(
  ...
  const allHackers = projects.flatMap((p) =>
    (p.collaborators || [])
      .filter((c) => ["HACKER", "PROJECT_ADMIN", "PENTESTER"].includes(c.role))
      .map((c) => c.userId),
  );
  const uniqueHackerCount = new Set(allHackers).size;

  const hackerAvatars = projects
    .flatMap((p) =>
      (p.collaborators || [])
        .filter((c) =>
          ["HACKER", "PROJECT_ADMIN", "PENTESTER"].includes(c.role),
        )
        .map((c) => c.user?.fullName?.[0] || "H"),
    )
    .slice(0, 4);

  const openFindingsCount = projects.reduce(
    (acc, p) => acc + (p._count?.findings || 0),
    0,
  );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <StatCard
          ...
          color="text-[#00c477]"
          progress={
            projects.length > 0 ? (activeCount / projects.length) * 100 : 0
          }
          trend={
            projects.length > 0 ? `${activeCount} ACTIVE` : "NO ACTIVE PROJECTS"
          }
        />
        <StatCard
