...
const OrgOnboarding = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post(
        `/organizations/${orgId}/submit-verification`,
        updatePayload,
      );
      ...
      toast.error(
        error.response?.data?.error ||
          "Failed to setup organization. Please try again.",
      );
      setLoading(false);
    }
  };
    ...
    <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-8 shadow-2xl backdrop-blur-md">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 text-xs font-mono uppercase tracking-widest text-gray-500 mb-6">
          <span className={step >= 1 ? "text-sky-400" : ""}>
            1. Business Profile
          </span>
          <FiChevronRight />
          <span className={step >= 2 ? "text-sky-400" : ""}>
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="bg-sky-500/5 border border-sky-500/20 rounded-xl p-4 mb-6">
              <p className="text-xs text-gray-400 leading-relaxed">
                To ensure a trusted ecosystem, we require verifiable
                registration details before you can launch pentest programs.
              </p>
            </div>
