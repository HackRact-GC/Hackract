            ...
            <div key={dateLabel} className="space-y-3">
              ...
              <div className="px-4 space-y-4">
                {records.map((record, index) => {
                  const recordUserId =
                    record.userId || record.user?.id || record.user?._id;
                  const isYou =
                    localUserId && String(recordUserId) === String(localUserId);
                  const userName =
                    record.user?.fullName || record.user?.name || "Agent X";
                      ...
                      <div className="bg-[#161a23] border border-gray-800/60 rounded-md p-2.5 hover:border-gray-700 transition-colors ml-2 relative">
                        <div className="flex justify-between items-start mb-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            {isLive && !isYou && (
                              <span className="text-[8px] bg-[#00ff41]/20 text-[#00ff41] px-1 rounded uppercase font-bold animate-pulse">
                                New
                              </span>
                            )}
                          </div>
                        ...
                        {record.isSnapshot && record.snapshot && (
                          <button
                            onClick={() =>
                              onRestore && onRestore(record.snapshot)
                            }
