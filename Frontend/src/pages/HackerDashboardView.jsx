...
const StatCard = ({ label, value, trend, icon: Icon, color }) => (
  <div className="bg-black/70 border border-white/10 p-6 rounded-4xl flex flex-col gap-4 group hover:border-[#00c477]/30 transition-all duration-300 backdrop-blur-md">
    <div className="flex items-center justify-between">
      <div
        className={`w-12 h-12 rounded-2xl bg-black flex items-center justify-center shrink-0 border border-white/10 group-hover:border-[#00c477]/30 transition-colors`}
      >
...
const Badge = ({ text, type = "default" }) => {
  ...
  return (
    <span
      className={`inline-block text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md border ${styles[type] || styles.default}`}
    >
      {text}
    </span>
  );
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-black/70 rounded-[2.5rem] border border-white/10 overflow-hidden flex flex-col backdrop-blur-md">
          <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between bg-white/5">
            ...
            <button className="text-[10px] font-bold text-[#00c477] hover:text-white transition-colors uppercase tracking-widest">
              Execute Full Scan
            </button>
          </div>
          <div className="p-6 space-y-4">
            {[
              ...
              <div
                key={m.id}
                className="p-6 rounded-3xl bg-black border border-white/10 hover:border-[#00c477]/30 hover:bg-white/5 transition-all cursor-pointer group"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-white group-hover:text-[#00c477] transition-colors uppercase tracking-tight">
                      {m.name}
                    </p>
                  ...
                  <Badge
                    text={m.status}
                    type={m.status === "Ongoing" ? "info" : "success"}
                  />
                </div>
        ...
        <div className="bg-black/70 rounded-[2.5rem] border border-white/10 overflow-hidden backdrop-blur-md">
          <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between bg-white/5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono text-[#00c477] uppercase tracking-widest">
                Synced
              </span>
            </div>
          </div>
          <div className="p-8 space-y-8 relative">
            ...
            {[
              {
                id: 1,
                action: "Exploit Validated",
                target: "SQLi in AuthService",
                time: "2h ago",
                icon: FiTarget,
              ...
              {
                id: 3,
                action: "Reward Dispatched",
                target: "$2,500.00 BTC equiv.",
                time: "1d ago",
                icon: FiDollarSign,
                color: "text-[#00c477] bg-[#00c477]/10 border-[#00c477]/20",
              },
              {
                id: 4,
                action: "Rank Calibrated",
                target: "Advanced to Elite Tier",
                time: "3d ago",
                icon: FiAward,
                color: "text-[#00c477] bg-[#00c477]/10 border-[#00c477]/20",
              },
              ...
              <div key={a.id} className="flex gap-6 items-start relative z-10">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${a.color} shadow-lg`}
                >
                  <a.icon size={18} />
                </div>
