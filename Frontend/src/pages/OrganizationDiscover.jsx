...
const RankBadge = ({ rank }) => {
  ...
  return (
    <span
      className={`text-[9px] font-black font-mono uppercase tracking-widest px-2 py-0.5 rounded border ${colors[rank] || colors.BRONZE}`}
    >
      {rank}
    </span>
  );
...
const InviteModal = ({ hacker, onClose }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.success(
        `Invitation sent to ${hacker.user?.handle || hacker.handle || hacker.name}!`,
      );
      onClose();
    } catch (err) {
      const msg =
  ...
  const hackerName =
    hacker.user?.fullName || hacker.fullName || hacker.name || "Unknown";
  const hackerHandle = hacker.user?.handle || hacker.handle || hacker.tag || "";
  const hackerAvatar =
    hacker.user?.avatar ||
    hacker.avatar ||
    `https://api.dicebear.com/7.x/bottts/svg?seed=${hackerHandle}&baseColor=00ff88`;
        ...
        <div className="px-8 pt-8 pb-6 border-b border-white/5 flex items-start gap-4 relative z-10">
          <div className="relative w-14 h-14 shrink-0">
            <img
              src={hackerAvatar}
              alt={hackerName}
              className="w-full h-full rounded-xl bg-black/50 border border-white/10 object-cover"
            />
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-black text-[#00c477] font-mono tracking-widest uppercase mb-1">
        ...
        <div className="px-8 py-6 space-y-5 relative z-10">
          ...
          <div>
            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <FiMessageSquare className="text-[#00c477]" /> Personal Message{" "}
...
const HackerCard = ({ hacker, index, onInvite, onViewProfile, authUser }) => {
  ...
  const avatar =
    hacker.user?.avatar ||
    `https://api.dicebear.com/7.x/bottts/svg?seed=${handle}&baseColor=00ff88`;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="relative w-16 h-16 rounded-xl bg-linear-to-br from-[#00c477]/20 to-emerald-900/40 p-0.5 border border-white/10 group-hover:border-[#00c477]/50 transition-colors">
          <img
            src={avatar}
            alt={name}
            className="w-full h-full rounded-[10px] object-cover bg-black/50"
          />
        ...
        <div className="flex flex-col items-end gap-1.5">
          <div className="flex items-center gap-1 text-white font-bold">
            <FiStar
              className={`text-sm ${rating ? "text-yellow-500 fill-yellow-500" : "text-gray-600"}`}
            />
      ...
      {/* Name */}
      <div className="mb-4">
        <h3 className="text-lg font-bold text-white group-hover:text-[#00c477] transition-colors mb-0.5">
          {name}
        </h3>
        {handle && (
          <p className="text-xs text-gray-500 font-mono">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {skills.slice(0, 3).map((s) => (
          <span
            key={s}
            className="px-2.5 py-1 rounded border border-white/10 bg-white/5 text-[10px] text-gray-300 font-mono"
          >
            {s}
          </span>
        ))}
        {certs.slice(0, 2).map((c) => (
          <span
            key={c}
            className="px-2.5 py-1 rounded border border-[#00c477]/20 bg-[#00c477]/5 text-[10px] text-[#00c477] font-mono"
          >
            {c}
          </span>
        ))}
        {skills.length + certs.length > 5 && (
          <span className="px-2.5 py-1 rounded border border-white/5 bg-white/2 text-[10px] text-gray-600 font-mono">
            +{skills.length + certs.length - 5} more
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5 mt-auto">
        <div className="bg-white/2 border border-white/5 rounded-xl p-3 flex flex-col justify-between h-[68px]">
          <span className="text-[9px] font-black text-gray-500 tracking-wider uppercase font-mono block mb-1">
            Reputation
          </span>
...
const OrganizationDiscover = () => {
  ...
  const fetchHackers = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCerts.length > 0)
        params.set("certs", selectedCerts.join(","));
  ...
  const filteredHackers = hackers.filter((h) => {
    ...
    if (
      ...
      !selectedCerts.some((c) => certs.includes(c))
    )
      return false;
    ...
    <div className="flex flex-col h-full -m-10">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 border-r border-white/5 bg-[#050505] p-8 overflow-y-auto hidden md:block">
          <h3 className="text-[10px] font-black text-[#00c477] tracking-widest font-mono mb-8 uppercase">
            Refine Discovery
          </h3>

          <div className="mb-8">
            <h4 className="text-[9px] font-black text-gray-500 tracking-widest font-mono mb-4 uppercase">
              Core Skills
            </h4>
            <div className="space-y-3">
              {[
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isActive ? "bg-[#00c477] border-[#00c477]" : "border-white/20 bg-black/50 group-hover:border-[#00c477]/50"}`}
                    >
                      {isActive && (
                        <FiCheck className="text-black text-[10px] font-bold" />
                      )}
                    </div>
                    <span
                      className={`text-xs transition-colors ${isActive ? "text-white font-bold" : "text-gray-400 group-hover:text-gray-200"}`}
                    >
                      {skill}
                    </span>
                  </label>
          ...
          <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight mb-2">
                Top Penetration Experts
              </h1>
              <p className="text-gray-400 text-sm">
                {loading
                  ? "Loading..."
                  : `Showing ${filteredHackers.length} verified security researchers.`}
              </p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-full border border-[#00c477]/20 bg-[#00c477]/5">
              ...
              <span className="text-[10px] font-mono font-bold text-[#00c477] uppercase tracking-widest">
                Live_Datafeed: Synced
              </span>
            </div>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-[#0a0a0a] border border-white/5 rounded-xl p-6 animate-pulse h-64"
                >
