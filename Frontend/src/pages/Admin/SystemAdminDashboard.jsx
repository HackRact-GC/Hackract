...
const SystemAdminDashboard = (props = {}) => {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      sender: "AI",
      text: "Hello Admin. System is ready. How can I assist you with the Project ?",
    },
  ]);
  ...
  const handleSendMessage = (e) => {
    ...
    setTimeout(() => {
      setChatMessages((prev) => [
        ...prev,
        { sender: "AI", text: "Analyzing request... Please wait." },
      ]);
    }, 1000);
  };
  ...
  const completedCount = timelinePhases.filter(
  ...
  const getPhaseIcon = (status) => {
    ...
    if (status === "start")
      return <FiClock size={16} strokeWidth={2} className="animate-pulse" />;
    return <div className="w-2 h-2 rounded-full bg-gray-500"></div>;
  };

  const getPhaseColor = (status) => {
    if (status === "completed")
      return "bg-[#4ade80] text-black shadow-[0_0_15px_rgba(74,222,128,0.3)] hover:bg-[#3bca71]";
    if (status === "start")
      return "bg-[#38bdf8] text-black shadow-[0_0_15px_rgba(56,189,248,0.3)] hover:bg-[#2caae0]";
  ...
  return (
    <div className="h-full text-gray-300 font-sans p-6 overflow-y-auto">
      ...
      >
            ...
            <div className="space-y-4">
              {loadingHistory ? (
                ...
                <div className="text-gray-600 italic text-xs">
                  NO WORKFLOW DATA CAPTURED. STANDBY...
                </div>
              ) : (
                history.map((item) => {
                  const isFinding =
                    item.action === "LINK_FINDING" ||
                    item.message?.toLowerCase().includes("finding");
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`text-gray-400 text-xs leading-relaxed border-l pl-3 py-1 hover:bg-white/5 transition-colors ${isFinding ? "border-red-500/30" : "border-[#2a3036]"}`}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-gray-600 font-bold">
                          [{new Date(item.createdAt).toLocaleTimeString()}]
                        </span>
                        <span className="text-[#38bdf8] font-black uppercase tracking-tighter">
                          {item.user?.handle || "SYS"}
                        </span>
                        <span
                          className={`${actionColor} px-1.5 rounded text-[8px] font-black uppercase border`}
                        >
                          {isFinding ? "VULNERABILITY" : item.action}
                        </span>
                      </div>
                      <div
                        className={`${isFinding ? "text-red-400" : "text-gray-300"}`}
                      >
                        {item.message}
                        {item.workflow?.name && (
        ...
        >
          <div
            className={`w-[2px] h-12 rounded-full transition-colors ${isResizing ? "bg-[#00c477]" : "bg-[#2a3036] group-hover:bg-gray-500"}`}
          />
        </div>
          ...
          <div className="bg-[#1c1f24] border border-[#2a3036] rounded-xl flex flex-col flex-1 overflow-hidden relative shadow-lg">
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-gradient-to-b from-[#1c1f24] to-[#16191d]">
              {chatMessages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`flex items-start max-w-[80%] space-x-2 ${msg.sender === "Admin" ? "flex-row-reverse space-x-reverse" : ""}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-md ${msg.sender === "Admin" ? "bg-gray-700" : "bg-[#38bdf8]/20 text-[#38bdf8] border border-[#38bdf8]/30"}`}
                    >
                      {msg.sender === "Admin" ? <FiUser size={14} /> : "AI"}
                    </div>
                    <div
                      className={`px-4 py-2 rounded-2xl text-sm shadow-md ${msg.sender === "Admin" ? "bg-[#2a3036] text-white rounded-tr-sm" : "bg-[#1a2026] border border-white/5 text-gray-300 rounded-tl-sm"}`}
                    >
                      {msg.text}
                    </div>
                  </div>
