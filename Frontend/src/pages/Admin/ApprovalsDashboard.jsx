...
const ApprovalsDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const pendingHackers = (hackerRes.data.data || []).filter(
        (h) => h.status === "SUBMITTED",
      );
      const pendingOrgs = (orgRes.data.data || []).filter(
        (o) => o.verificationStatus === "SUBMITTED",
      );
            ...
            {activeTab === "hackers" &&
              (hackers.length === 0 ? (
                ...
                hackers.map((hacker) => (
                  <div
                    key={hacker.id}
                    className="bg-white/5 border border-white/10 p-6 rounded-xl flex items-center justify-between hover:border-sky-500/50 transition-colors group"
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <h3 className="text-lg font-bold">
                          {hacker.user?.fullName || "Unknown User"}
                        </h3>
                        <div className="flex items-center gap-4 text-sm text-gray-400 mt-1">
                          <span className="font-mono">
                            @
                            {hacker.user?.handle ||
                              hacker.userId.substring(0, 8)}
                          </span>
                    ...
                    <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() =>
                          navigate(`/admin/approvals/hacker/${hacker.id}`)
                        }
                      ...
                      <button
                        onClick={() =>
                          handleAction("hacker", hacker.id, "reject")
                        }
                      ...
                      <button
                        onClick={() =>
                          handleAction("hacker", hacker.id, "approve")
                        }
            ...
            {activeTab === "orgs" &&
              (orgs.length === 0 ? (
                ...
                orgs.map((org) => (
                  <div
                    key={org.id}
                    className="bg-white/5 border border-white/10 p-6 rounded-xl flex items-center justify-between hover:border-[#00c477]/50 transition-colors group"
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-400 mt-1">
                          <span className="font-mono">
                            {org.taxId ? `TAX ID: ${org.taxId}` : "NO TAX ID"}
                          </span>
                          <span className="flex items-center gap-1 text-[#00c477]">
                            <FiClock size={12} />{" "}
                    ...
                    <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() =>
                          navigate(`/admin/approvals/org/${org.id}`)
                        }
                        className="p-2 border border-white/20 rounded hover:bg-white/10 bg-transparent text-gray-300"
                        title="View Details"
                      >
