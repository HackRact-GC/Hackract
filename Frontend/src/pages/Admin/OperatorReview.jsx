...
const OperatorReview = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const { data } = await api.get("/hacker-profiles", {
        params: { limit: 100 },
      });
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#0a0f16] p-8 text-white font-mono">
        LOADING PROFILE...
      </div>
    );
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-white/5 border border-white/10 p-6 rounded-2xl flex flex-col items-center text-center">
              ...
              <p className="text-gray-400 text-xs mt-4 uppercase tracking-widest">
                {profile.country || "No Country Specified"}
              </p>
              ...
              <div className="w-full mt-6 pt-6 border-t border-white/10 text-left space-y-3">
                <div>
                  ...
                  <p className="text-gray-200 font-mono mt-1">
                    {profile.idDocumentNumber || "Not Provided"}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500 text-xs font-mono uppercase">
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.primarySkills?.length > 0 ? (
                    profile.primarySkills.map((skill) => (
                      <span
                        key={skill}
                        className="bg-sky-500/10 text-sky-400 border border-sky-500/20 px-3 py-1 rounded-full text-xs font-mono uppercase"
                      >
                        {skill}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.certifications?.length > 0 ? (
                    profile.certifications.map((cert) => (
                      <span
                        key={cert}
                        className="bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 px-3 py-1 rounded-full text-xs font-mono uppercase"
                      >
                        {cert}
                      </span>
                    ))
