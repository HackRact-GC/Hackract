...
const Icons = {
  ...
  ShoppingBag: ({ className = "w-5 h-5", ...props }) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
  ...
  Trophy: () => (
    <svg
      ...
      className="w-5 h-5"
    >
      <path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6M18 9h1.5a2.5 2.5 0 0 0 0-5H18M4 22h16M10 14.66V17c0 .55.45 1 1 1h2c.55 0 1-.45 1-1v-2.34M12 15a5 5 0 0 0 5-5V4H7v6a5 5 0 0 0 5 5z" />
    </svg>
  ),
  ChevronRight: () => (
    <svg
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl rounded-[32px] border border-white/5 p-7 flex flex-col gap-5 hover:border-[#00c477]/30 hover:-translate-y-1.5 transition-all duration-500 group shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center justify-between relative z-10">
        <div
          className={`w-14 h-14 rounded-2xl ${bg[color]} flex items-center justify-center shrink-0 border shadow-[0_0_15px_rgba(0,0,255,136,0.05)]`}
        >
          <Icon />
        </div>
        {trend && (
...
const Badge = ({ text, type = "default" }) => {
  ...
  return (
    <span
      className={`inline-block text-[9px] font-mono font-black uppercase tracking-widest px-3 py-1 rounded-lg border ${styles[type] || styles.default}`}
    >
      {text}
    </span>
  );
...
const HackerDashboard = () => {
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="text-[11px] font-mono font-black text-[#00c477] uppercase tracking-[0.4em] animate-pulse">
            Initializing Secure Intel Link...
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          value={realProjects.filter((p) => !p.isPersonal).length}
          trend={realProjects.length > 0 ? "Live" : "Idle"}
          icon={Icons.Target}
          color="rose"
        />
        <StatCard
        ...
        <div className="bg-white/[0.02] backdrop-blur-3xl rounded-[40px] border border-white/5 overflow-hidden flex flex-col shadow-2xl relative">
          ...
          <div className="px-10 py-8 flex items-center justify-between border-b border-white/5">
            ...
            <button
              ...
              className="text-[10px] font-mono font-bold text-gray-500 hover:text-[#00c477] uppercase tracking-widest transition-colors"
            >
              Operational View [+]
            </button>
          </div>
              ...
              realProjects
                ...
                .map((m) => (
                  <div
                    ...
                    className="p-6 rounded-[28px] bg-white/[0.02] border border-transparent hover:border-[#00c477]/20 hover:bg-white/[0.04] transition-all cursor-pointer group"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-base font-black text-white group-hover:text-[#00c477] transition-colors uppercase tracking-tight mb-1">
                          {m.name}
                        </p>
                        <p className="text-[10px] font-mono font-bold text-gray-500 uppercase tracking-widest">
                          {m.organization?.name || "Private Org"}
                        </p>
                      </div>
                      <Badge
                        text={m.status || "PLANNING"}
                        type={m.status === "IN_PROGRESS" ? "success" : "info"}
                      />
                    </div>
                    <div className="flex items-center gap-6 pt-4 border-t border-white/5">
                      <div className="flex items-center gap-2 text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest">
                        <span className="text-[#00c477]">
                          <Icons.Zap />
                        </span>{" "}
                        Scope: {m.isPersonal ? "Local" : "Org"}
                      </div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[11px] font-mono font-bold text-gray-500 uppercase tracking-widest">
            Unclaimed technical challenges across the global spectrum.
          </p>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button className="px-8 py-3.5 bg-[#00c477] text-black text-[11px] font-mono font-black uppercase tracking-widest rounded-[18px] shadow-[0_0_20px_rgba(0,255,136,0.3)] hover:scale-105 transition-all">
            Filter Archive
          </button>
        </div>
      </div>
      ...
      return (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white/[0.02] rounded-[28px] border border-white/5 p-8 animate-pulse h-36"
            />
          ))}
        </div>
      );
        ...
        <div className="flex flex-col items-center justify-center py-32 gap-6 text-center">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm max-w-sm font-mono">
              When organizations invite you to their security programs, they'll
              appear here.
            </p>
          </div>
        </div>
          ...
          return (
            <motion.div
              ...
              className={`bg-white/[0.02] backdrop-blur-xl rounded-[28px] border p-8 transition-all ${
                isPending
                  ? "border-[#00c477]/20 hover:border-[#00c477]/40"
                  : "border-white/5"
              }`}
            >
              {/* Header */}
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span
                      className={`text-[9px] font-black font-mono uppercase tracking-widest px-2 py-0.5 rounded border ${STATUS_STYLES[inv.status] || STATUS_STYLES.PENDING}`}
                    >
                      {inv.status}
                    </span>
                    {org && (
  ...
  const TABS = {
    ...
    engagements: OverviewTab,
    earnings: OverviewTab,
    marketplace: MarketplaceTab,
    rankings: OverviewTab,
    ...
    <div className="min-h-screen bg-[#050505] font-sans text-white selection:bg-[#00c477]/30 selection:text-[#00c477] overflow-x-hidden">
      ...
      <header className="h-24 bg-black/40 backdrop-blur-3xl border-b border-white/5 flex items-center px-12 sticky top-0 z-[60]">
        <div className="flex items-center gap-8">
          <div
            className="text-2xl font-black tracking-[0.2em] text-white flex items-center gap-4 group cursor-pointer"
            onClick={() => navigate("/")}
          >
            <span className="w-12 h-12 bg-black border border-[#00c477]/30 text-[#00c477] rounded-xl flex items-center justify-center font-mono shadow-[0_0_15px_rgba(0,255,136,0.15)] group-hover:shadow-[0_0_25px_rgba(0,255,136,0.3)] transition-all">
              λ
            </span>
            HACKRACT{" "}
            <span className="text-[10px] font-mono font-black text-[#00c477]/60 uppercase tracking-[0.4em] bg-[#00c477]/5 px-3 py-1.5 rounded-lg border border-[#00c477]/10 ml-2">
              CORE.v2
            </span>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-10">
          <div className="flex items-center gap-8 pl-10">
            ...
            <button
              ...
              className="flex items-center gap-5 bg-white/[0.02] border border-white/5 rounded-2xl pl-5 pr-4 py-2 hover:border-[#00c477]/40 hover:bg-white/[0.04] transition-all group shadow-xl"
            >
              <div className="text-right hidden sm:block">
                <p className="text-[9px] font-mono font-black text-gray-600 uppercase tracking-[0.2em] leading-none mb-1.5">
                  Authorized
                </p>
                <p className="text-sm font-black text-white group-hover:text-[#00c477] transition-colors uppercase leading-none tracking-tight">
                  Operator.v1
                </p>
              </div>
              <div className="w-11 h-11 rounded-xl bg-black border border-[#00c477]/30 text-[#00c477] flex items-center justify-center font-mono font-black text-xs shadow-inner shadow-[#00c477]/10 group-hover:shadow-[0_0_15px_rgba(0,255,136,0.2)] transition-all">
                λ
              </div>
            </button>
          </div>
      ...
      <div className="max-w-[1720px] mx-auto flex gap-16 px-12 py-16 relative z-10">
        ...
        <aside className="w-80 shrink-0 space-y-2.5">
          <div className="text-[10px] font-mono font-black text-gray-600 uppercase tracking-[0.5em] mb-6 pl-4">
            Tactical Matrix
          </div>
          {NAV.map((item) => (
            <button
            ...
            >
              <span
                className={
                  activeTab === item.key
                    ? "text-[#00c477] drop-shadow-[0_0_8px_rgba(0,255,136,0.6)]"
          ...
          <div className="mt-20 p-10 rounded-[48px] bg-gradient-to-br from-[#004e2b] to-[#012e1a] border border-[#00c477]/20 text-white relative overflow-hidden shadow-2xl group cursor-pointer hover:border-[#00c477]/40 transition-all">
            ...
            <div className="relative z-10">
              <p className="text-[9px] font-mono font-black uppercase tracking-[0.4em] text-[#00c477] mb-3 opacity-80">
                Encryption Key
              </p>
              <h4 className="text-xl font-extrabold leading-tight mb-8 uppercase tracking-tight">
                Unlock Elite <br />
                Neural Links
              </h4>
              <button className="w-full py-4.5 bg-[#00c477] text-black text-[10px] font-mono font-black uppercase tracking-[0.2em] rounded-2xl hover:scale-105 transition-all shadow-[0_0_20px_rgba(0,255,136,0.3)] border-none">
                Initialize Upgrade
              </button>
            </div>
          </div>
