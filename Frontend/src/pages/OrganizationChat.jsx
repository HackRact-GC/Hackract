...
const TypingDots = () => (
  <div className="flex items-center gap-1 py-1">
    {[0, 1, 2].map((i) => (
      <motion.span
        key={i}
        className="w-1.5 h-1.5 rounded-full bg-[#00c477] block"
        animate={{ y: [0, -5, 0] }}
...
export default function OrganizationChat() {
  ...
  const handleStopTyping = useCallback((userId, conversationId) => {
    setTypingMap((prev) => {
      const u = { ...(prev[conversationId] || {}) };
      delete u[userId];
      return { ...prev, [conversationId]: u };
    });
  }, []);
  ...
  const openConversation = useCallback(
    async (conv) => {
      if (prevConvId.current) {
        leaveConv(prevConvId.current);
        emitTypingStop(prevConvId.current);
      }
      setActive(conv);
  ...
  const handleDelete = useCallback(
    async (messageId) => {
      if (!active) return;
      try {
        await chatApi.deleteMessage(active.id, messageId);
        setMessages((prev) =>
          prev.map((m) =>
            m.id === messageId
  ...
  const activeOther =
    active?.type === "DIRECT"
      ? active.participants?.find((p) => p.userId !== user?.id)?.user
      : null;
  const isOtherOnline = activeOther
    ? (presenceMap[activeOther.id]?.isOnline ?? false)
    : false;
  const lastSeen = activeOther ? presenceMap[activeOther.id]?.lastSeenAt : null;
  const typingIds = active
    ? Object.keys(typingMap[active.id] || {}).filter((id) => id !== user?.id)
    : [];

  return (
    <div
      className="flex h-full bg-[#050505] overflow-hidden"
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#050505] border-l border-white/5">
          ...
          <div className="px-8 py-4 border-b border-white/5 flex items-center justify-between bg-[#050505]/80 backdrop-blur-xl shrink-0">
            <div className="flex items-center gap-4">
              {active.type === "GROUP" ? (
                <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white font-mono font-black text-xs uppercase tracking-widest">
                  {getInitials(active.name || "G")}
                </div>
              ) : (
                <Avatar
                  user={activeOther}
              ...
              <div>
                <h2 className="text-white font-black text-xs uppercase tracking-[0.2em] font-mono leading-none mb-1.5">
                  {active.type === "GROUP"
                    ? active.name
                    : activeOther?.fullName || activeOther?.handle}
                </h2>
                <div className="flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${isOtherOnline ? "bg-[#00c477] shadow-[0_0_8px_#00c477]" : "bg-gray-700"}`}
                  />
          ...
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-4 font-mono">
            {hasMore && (
              <div className="flex justify-center pb-4">
                <button
                  onClick={loadMore}
                  className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 hover:text-[#00c477] transition-colors"
                >
            ...
            {typingIds.length > 0 && (
              <div className="text-[9px] text-[#00c477] font-black uppercase tracking-widest animate-pulse">
                Researching_Input...
              </div>
            )}
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-12 bg-[#050505]">
          <div className="text-center space-y-4 max-w-sm">
            <div className="inline-flex p-4 rounded-2xl bg-white/2 border border-white/5 mb-4">
              <svg
                className="w-10 h-10 text-[#00c477]"
                fill="none"
                stroke="currentColor"
                strokeWidth="1"
                viewBox="0 0 24 24"
            ...
            <h2 className="text-xs font-black text-white uppercase tracking-[0.4em] font-mono">
              Secure_Comms_v2.0
            </h2>
            <p className="text-[10px] text-gray-600 font-mono leading-loose uppercase tracking-widest">
              Establish encrypted tunnels with verified security researchers.
              Peer-to-peer data exchange active.
            </p>
          </div>
        </div>
      )}
