...
import { 
  ...
  FiCheck, FiX, FiLock, FiUsers, FiFolder, FiChevronRight 
} from "react-icons/fi";

// ── Icons (Custom SVG for unique brand feel) ───────────────────────────────
const Icons = {
  Plus: () => <FiPlus className="w-4 h-4"/>,
  Folder: () => <FiFolder className="w-5 h-5"/>,
  Users: () => <FiUsers className="w-4 h-4"/>,
  Shield: () => <FiShield className="w-4 h-4"/>,
  ChevronRight: () => <FiChevronRight className="w-3.5 h-3.5"/>,
...
const StatusBadge = ({ status }) => {
  const map = {
    PLANNING:    "text-amber-400 bg-amber-500/10 border-amber-500/20 shadow-[0_0_15px_rgba(251,191,36,0.1)]",
    IN_PROGRESS: "text-[#00ff88] bg-[#00ff88]/10 border-[#00ff88]/20 shadow-[0_0_15px_rgba(0,255,136,0.1)]",
    REPORTING:   "text-violet-400 bg-violet-500/10 border-violet-500/20 shadow-[0_0_15px_rgba(139,92,246,0.1)]",
    CLOSED:      "text-gray-500 bg-gray-500/10 border-gray-500/20",
  };
  return (
    <span className={`inline-block text-[9px] font-mono font-black uppercase tracking-[0.2em] px-2.5 py-1 rounded-lg border ${map[status] || map.PLANNING}`}>
      {status?.replace("_", " ")}
    </span>
  );
};
...
const ProjectCard = ({ project, onClick }) => (
  ...
  >
    ...
    <h3 className="text-xl font-black text-white mb-3 uppercase tracking-tight group-hover:text-[#00ff88] transition-colors truncate">{project.name}</h3>
    ...
    <div className="flex items-center justify-between pt-8 border-t border-white/5">
      <div className="flex items-center gap-6">
        ...
        {project.organization && <div className="flex items-center gap-2 text-[9px] font-mono font-black text-gray-600 uppercase tracking-widest"><Icons.Shield/>{project.organization.name}</div>}
      </div>
      <div className="opacity-0 group-hover:opacity-100 transition-all text-[#00ff88] translate-x-2 group-hover:translate-x-0">
        <Icons.ChevronRight/>
      </div>
    </div>
  </motion.button>
...
const Projects = () => {
  ...
  const loadOrganizations = async () => {
    ...
    if (orgList.length > 0) {
      const initialId = orgList[0].organizationId || orgList[0].id;
  ...
  const loadProjects = async (orgId) => {
    try {
      ...
      toast.error("Directive synchronization failure.");
    }
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (orgs.length) {
          const firstOrgId = orgs[0].organizationId || orgs[0].id;
          await loadProjects(firstOrgId);
          await loadOrgMembers(firstOrgId);
        }
      } catch (error) {
  ...
  const createProject = async e => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.error || "Directive instantiation failure.");
    } finally {
      setSubmitting(false);
    }
  };
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {organizations.map((org) => (
                        <option key={org.organizationId || org.id} value={org.organizationId || org.id}>{org.organization?.name || org.name}</option>
                      ))}
                    </select>
                  </div>
                      ...
                      {orgMembers.map((m) => (
                        <label key={m.userId} className={`flex items-center gap-4 p-3 rounded-xl border transition-all cursor-pointer font-mono group
                          ...
                          <div className={`w-4 h-4 rounded border transition-all flex items-center justify-center
                            ...
                            {form.hackerIds.includes(m.userId) && <FiCheck className="w-2.5 h-2.5 text-black"/>}
                          </div>
