...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider uppercase border ${cfg.color} ${cfg.bg} ${cfg.border}`}
    >
      <span
...
const PersonalProjectCard = ({ project, onOpen, onDelete, index }) => (
  ...
  >
    ...
    <p className="text-sm text-gray-500 leading-relaxed line-clamp-2 mb-6">
      {project.description ||
        "No specific target scope defined. This is an independent local runtime."}
    </p>
    ...
    <div className="mt-auto pt-4 border-t border-white/5 text-[12px] text-gray-400 flex items-center justify-between font-medium">
      <span className="flex items-center gap-1.5">
        ...
        {project.collaborators?.length || 0} Operators
      </span>
      {project.createdAt && (
        <span className="flex items-center gap-1.5">
          <FiClock size={14} className="text-gray-500" /> Init:{" "}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          <h3
            className={`text-lg font-bold ${isPending ? "text-white" : "text-white group-hover:text-[#00c477]"} transition-colors leading-snug truncate max-w-[200px]`}
          >
            {getProjectDisplayName(project)}
          </h3>
          <p className="text-[12px] text-gray-400 font-medium mt-0.5">
            {project.organization?.name || project.orgName || "Private Org"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-gray-300 font-bold">
            {project.findings || project._count?.findings || 0}{" "}
            <span className="font-normal text-gray-500">Vulns</span>
          </span>
        </div>
        <div
          className={`flex items-center gap-1.5 text-[12px] font-medium ${isPending ? "text-amber-500" : "text-gray-400"}`}
        >
          <FiClock size={13} />
          {project.deadline ||
            (project.endDate
      ...
      <div className="flex items-center justify-between gap-3 pt-2">
        ...
        <button
          onClick={(e) => {
            ...
            if (
              window.confirm(
                "Are you sure you want to withdraw your proposal for this project?",
              )
            ) {
...
const PersonalWorkspaceCard = ({ onCreate }) => {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/projects/personal", {
        name: name.trim(),
      ...
      toast.error(
        err?.response?.data?.error || "Failed to initialize workspace",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <motion.div
      layout
      className="border border-white/5 bg-[#050505] rounded-3xl overflow-hidden relative group font-sans"
    >
      <div className="p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6 relative z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Create personal workflow for independent exploitation testing,
            research, and personal tooling.
          </p>
        </div>
        <button
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between pt-2 gap-4">
                <div className="flex items-center gap-3 text-xs text-gray-500 font-medium">
                  ...
                  <span>
                    Private by default. Unlisted from org directories.
                  </span>
                </div>
                <button
...
const Projects = () => {
  ...
  const displayPersonal =
    activeTab === "ALL_ACCESS" || activeTab === "LOCAL_LABS";
  const displayOrg = activeTab === "ALL_ACCESS" || activeTab === "MISSIONS";
  const displayPending =
    activeTab === "ALL_ACCESS" || activeTab === "INBOUND_REQS";
  const displayApplied =
    activeTab === "ALL_ACCESS" || activeTab === "APPLICATIONS";
    ...
    <div className="flex flex-col h-full bg-[#050505] p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full text-gray-300 font-sans space-y-6 lg:space-y-8 min-h-[calc(100vh-80px)]">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm max-w-2xl">
          Manage your personal labs and track assigned organizational
      ...
      <div className="space-y-12 pb-12">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-200/70 mt-0.5 font-medium">
                      You have {pendingCount} outstanding mission request(s)
              ...
              {pendingInvitations.map((inv, i) => (
                <OrgProjectCard
                  ...
                  project={{
                    ...
                    orgName:
                      inv.pentest?.organization?.name || "Enterprise Org",
        ...
        {displayPersonal && (
          <section className="space-y-5">
            <h2 className="text-sm font-bold text-gray-400 tracking-wider uppercase flex items-center gap-3">
              <span className="w-2 h-2 rounded-full bg-blue-500" /> Independent
              Resources
            </h2>
        ...
        {displayOrg && (
          <section className="space-y-5">
            <h2 className="text-sm font-bold text-gray-400 tracking-wider uppercase flex items-center gap-3">
              <span className="w-2 h-2 rounded-full bg-[#00c477]" /> Active
        ...
        {displayApplied && (
          <section className="space-y-5">
            <h2 className="text-sm font-bold text-gray-400 tracking-wider uppercase flex items-center gap-3">
              <span className="w-2 h-2 rounded-full bg-amber-500" /> Sent
              Proposals
            </h2>
