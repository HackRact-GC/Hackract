...
const FindingDetails = () => {
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-black text-white p-10 font-mono text-[10px] animate-pulse uppercase tracking-[0.3em] flex items-center justify-center">
        Initializing finding telemetry...
      </div>
    );
  if (!finding)
    return (
      <div className="min-h-screen bg-black p-10 flex items-center justify-center font-mono uppercase tracking-widest text-xs text-white/40">
        Sector Data Corrupted: Finding not found.
      </div>
    );
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-gray-500 uppercase">
            <span>
              Severity:{" "}
              <span
                className={`font-bold ${getSeverityColor(finding.severity)}`}
              >
                {finding.severity}
              </span>
            </span>
            <span>
              CVSS Score:{" "}
              <span className="text-[#00c477] font-bold">
                {finding.cvssScore?.toFixed(1) || "N/A"}
              </span>
            </span>
            <span>
              Project:{" "}
              <span className="text-white">{finding.pentest?.name}</span>
            </span>
            {finding.affectedAsset && (
              <span>
                Asset:{" "}
                <span className="text-white">{finding.affectedAsset}</span>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-6">
          <div className="bg-black/60 border border-white/10 rounded-2xl p-8 space-y-4 backdrop-blur-md">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 flex items-center gap-3">
              <FiAlertCircle size={14} className="text-[#00c477]" />{" "}
              ...
              <div className="grid gap-4 mt-4">
                {(() => {
                  ...
                  return proofs.map((proofUrl, idx) => {
                    const isImage =
                      proofUrl.match(/\.(jpeg|jpg|gif|png|webp)$/i) != null;
                    const isPdf = proofUrl.match(/\.(pdf)$/i) != null;
                    return (
                      <div
                        key={idx}
                        className="bg-black/50 p-4 rounded-xl border border-white/5 space-y-3"
                      >
                        {isImage && (
                          <div className="relative w-full overflow-hidden rounded-lg border border-[#00ff41]/20 group">
                            <img
                              src={proofUrl}
                              alt={`Evidence ${idx + 1}`}
                              className="w-full object-contain max-h-[400px] bg-black/80"
                            />
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <a
                                ...
                                className="px-4 py-2 bg-[#00ff41] text-black text-xs font-bold rounded-lg hover:scale-105 transition-transform"
                              >
                                View Full Image
                              </a>
                            </div>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 font-mono text-[10px] text-[#00c477] hover:underline break-all"
                        >
