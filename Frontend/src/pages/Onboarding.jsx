...
const Onboarding = () => {
  ...
  useEffect(() => {
    if (
      user?.roles?.[0]?.type === "PENTESTER" ||
      user?.roles?.[0]?.type === "PROJECT_ADMIN"
    ) {
    ...
    try {
      ...
      setError(
        err?.response?.data?.message ||
          "Internal system error. Failed to synchronize identity.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#fafbfc] flex flex-col items-center justify-center p-6 font-sans text-[#1a1c1e]">
      ...
      >
        ...
        <p className="text-lg text-gray-500 font-medium max-w-lg mx-auto mb-16 leading-relaxed">
          Your account is verified. Now, choose your operational directive to
          begin your mission.
        </p>
        ...
        {error && (
          <motion.div
            initial={{ scale: 0.9 }}
            animate={{ scale: 1 }}
            className="bg-rose-50 border border-rose-100 p-4 rounded-2xl mb-8 text-rose-600 text-sm font-bold"
          >
            {error}
          </motion.div>
        )}
        ...
        <div className="grid grid-cols-1 gap-8 px-4 max-w-xl mx-auto">
          ...
          >
            ...
            <h3 className="text-2xl font-black text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
              Technical Operative
            </h3>
            <p className="text-gray-500 font-medium leading-relaxed mb-8">
              Hunt for vulnerabilities, earn bounties, and secure the digital
              frontier as an elite ethical hacker.
            </p>
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-xs font-black text-indigo-900/40 uppercase tracking-widest">
            Synchronizing Protocol Identity...
          </p>
        </div>
      )}
    </div>
