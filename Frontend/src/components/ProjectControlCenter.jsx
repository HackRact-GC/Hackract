  ...
  const fetchHackers = async (query = "") => {
    ...
    try {
      ...
      const { data } = await api.get(
        `/hacker-profiles/discover?${params.toString()}`,
      );
  ...
  const sendInvite = async (hackerId) => {
    ...
    try {
      await invitationService.sendInvitation({
        ...
        hackerId,
        message:
          "You have been invited to collaborate on this security program.",
      });
      ...
      >
        <div className="p-8 border-b border-gray-800 shrink-0">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-black text-[#00c477] tracking-widest uppercase">
                Operator Discovery
              </h3>
              <p className="text-[10px] text-gray-500 font-mono tracking-[0.2em] mt-1 uppercase">
                RECRUIT TOP PENETRATION EXPERTS
              </p>
            </div>
            <button
              onClick={onClose}
              className="p-2 rounded-xl hover:bg-white/5 text-white/20 hover:text-[#00c477] transition-all"
            >
              <FiX size={20} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#050505]">
          {loading ? (
            <div className="py-20 flex justify-center">
              <div className="w-8 h-8 border-2 border-transparent border-t-[#00c477] rounded-full animate-spin" />
            </div>
          ) : results.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {results.map((hacker) => {
                ...
                const skills = parseItems(
                  hacker.primarySkills || hacker.skills,
                ).slice(0, 3);
                ...
                return (
                  <div
                    key={hacker.id}
                    className="flex flex-col justify-between p-5 bg-[#1a1d24] rounded-xl border border-gray-800 hover:border-[#00c477]/30 transition-all group"
                  >
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-200 tracking-wider uppercase truncate max-w-[150px]">
                              {name}
                            </p>
                            <p className="text-[10px] text-[#00c477] font-mono tracking-widest">
                              {handle ? `@${handle}` : "OPERATOR"}
                            </p>
                          </div>
                        </div>
                        <span className="text-[9px] px-2 py-0.5 rounded border border-gray-600 bg-gray-800 font-mono text-gray-300">
                          {rank}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {skills.length > 0 ? (
                          skills.map((s) => (
                            <span
                              key={s}
                              className="px-2 py-0.5 bg-black/50 border border-gray-700 rounded text-[9px] text-gray-400 font-mono"
                            >
                              {s}
                            </span>
                          ))
                    ...
                    <div className="flex items-center gap-2 pt-3 border-t border-gray-800">
                      <button
                        disabled={
                          sending === hacker.id || sending === hacker.userId
                        }
                      ...
                      >
                        {sending === (hacker.userId || hacker.id) ? (
                          <div className="w-3 h-3 border-2 border-transparent border-t-[#00c477] rounded-full animate-spin" />
                        ) : (
                          <FiSend />
                        )}
                        Invite
            ...
            <p className="text-center py-20 text-[10px] text-gray-600 uppercase tracking-widest font-mono">
...
const ProjectControlCenter = ({ projectId, onBack }) => {
  ...
  const loadProject = async () => {
    ...
    try {
      ...
      if (canManageInvitations) {
        try {
          const invRes =
            await invitationService.getInvitationsByProject(projectId);
          setInvitations(invRes.data || []);
        } catch (invError) {
          console.warn(
            "Could not load invitations:",
            invError.response?.status,
          );
  ...
  const handleMakeAdmin = async (userId) => {
    try {
      await api.patch(`/projects/${projectId}/admin`, {
      ...
      toast.error(
        err.response?.data?.message || "Failed to make lead pentester",
      );
    }
  };
  ...
  const handleRemoveAdmin = async () => {
    if (
      !window.confirm(
        "Are you sure you want to remove the Administrator designation for this operator?",
      )
    )
      return;
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to remove administrator designation",
      );
    }
  };
  ...
  const handleRemoveHacker = async (userId) => {
    if (
      !window.confirm(
        "Are you sure you want to remove this operator from the project?",
      )
    )
      return;
  ...
  const handleCreateWorkflow = async () => {
    ...
    try {
      ...
      if (res.data?.success || res.data?.id) {
        ...
        if (newWorkflowId)
          window.open(`/org-workflows/${newWorkflowId}`, "_blank");
      }
    } catch (err) {
  ...
  const currentPhaseIndex =
    statuses.indexOf(project.status) !== -1
      ? statuses.indexOf(project.status)
      : 1;
    ...
    <div className="bg-[#0f1115] min-h-screen text-gray-300 font-mono p-4 md:p-8 selection:bg-[#00c477]/30 overflow-x-hidden">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.2em] text-gray-500 mb-2 truncate">
          <button
            onClick={onBack}
            className="hover:text-[#00c477] transition-colors flex items-center gap-1"
          >
            <FiArrowLeft /> BACK
          </button>
          <span>
            / PROJECTS /{" "}
            {project.name?.replace(/\s+/g, "_")?.toUpperCase() || "NEXUS_CORE"}{" "}
            / <span className="text-[#00c477]">CONTROL</span>
          </span>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div>
            <h1 className="text-2xl font-black text-white tracking-widest uppercase break-all">
          ...
          >
            {project?.workflows?.[0] ? "OPEN_WORKFLOW" : "CREATE_WORKFLOW"}{" "}
            <FiPlus size={18} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#15181e] border border-gray-800 rounded-lg p-6 relative overflow-hidden">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="text-[#00c477] text-xs font-black uppercase tracking-widest mb-2">
                    MISSION_TIMELINE
                  </h3>
                  <div className="text-2xl text-gray-300">
                    Phase 0{currentPhaseIndex + 1}/04 -{" "}
                ...
                <div className="text-right">
                  ...
                  <div className="text-3xl font-black text-[#00c477]">
                    {phasePercentage}%
                  </div>
                </div>
              </div>
                ...
                {statuses.map((status, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left border-l-2 pl-3 transition-all ${
                        isActive
                    ...
                    >
                      ...
                      <div
                        className={`text-xs font-bold uppercase transition-colors ${
                          isActive
                            ? "text-gray-300 group-hover:text-[#00ff88]"
                            : "text-gray-700"
                        }`}
                      >
                        {status.replace("_", " ")}
            ...
            <div className="bg-[#15181e] border border-gray-800 rounded-lg p-6">
              <div className="flex justify-between items-center border-b border-gray-800 pb-4 mb-6">
                <h3 className="text-[#00c477] text-xs font-black uppercase tracking-widest">
                  OPERATIVE_FLEET
                </h3>
                <div className="flex items-center gap-4">
                  <div className="text-[10px] text-gray-500 uppercase tracking-widest">
                    {hackers.length}_ACTIVES_IDENTIFIED
                  </div>
                  <button
              ...
              <div className="space-y-4">
                {hackers.map((hacker) => {
                  ...
                  return (
                    <div
                      key={hacker.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-[#1a1d24] border border-gray-800/50 rounded hover:border-[#00c477]/30 transition-colors group gap-4"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="overflow-hidden">
                          <div className="text-sm font-bold text-gray-200 tracking-wider uppercase truncate">
                            {hacker.user?.fullName || hacker.user?.email}
                          </div>
                          <div className="flex items-center gap-3 mt-1 flex-wrap">
                            <span className="text-[9px] bg-[#1a3a2d] text-[#00c477] px-1.5 py-0.5 uppercase tracking-widest">
                              ACCEPTED
                            </span>
                ...
                {(Array.isArray(invitations) ? invitations : [])
                  ...
                  .map((inv) => (
                    <div
                      key={inv.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-[#1a1d24]/40 border border-dashed border-gray-700 rounded hover:border-amber-500/30 transition-colors group gap-4"
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative shrink-0">
                        ...
                        <div className="overflow-hidden">
                          <div className="text-sm font-bold text-gray-500 tracking-wider uppercase truncate">
                            {inv.hacker?.fullName || inv.hacker?.handle}
                          </div>
                          <div className="flex items-center gap-3 mt-1 flex-wrap">
                            <span className="text-[9px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 uppercase tracking-widest border border-amber-500/20">
                              PENDING_INVITE
                            </span>
                ...
                {hackers.length === 0 &&
                  (Array.isArray(invitations) ? invitations : []).filter(
                    (i) => i.status === "PENDING",
                  ).length === 0 && (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#15181e] border border-gray-800 rounded-lg p-6 flex flex-col h-[320px]">
              <h3 className="text-gray-400 text-xs font-black uppercase tracking-widest mb-4 shrink-0">
                LIVE_LOGS
              </h3>
