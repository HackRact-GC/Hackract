...
const ProjectActivity = ({ projectId }) => {
  ...
  const getActionIcon = (log) => {
    if (log.type === "finding") {
      return (
        <FiAlertTriangle className="text-rose-500 animate-pulse shrink-0" />
      );
    }
    switch (log.action) {
      case "PROJECT_CREATED":
        return <FiPlusCircle className="text-sky-400 shrink-0" />;
      case "HACKER_HIRED":
        return <FiUser className="text-[#00c477]" />;
  ...
  const formatAction = (log) => {
      ...
      return (
        <span className="space-y-1 block">
          <span className="flex items-center gap-2">
            <span className="text-rose-400 font-bold uppercase tracking-widest text-[9px]">
              NEW FINDING DETECTED:
            </span>
            <span
              className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${getSeverityStyle(log.severity)}`}
            >
              {log.severity}
            </span>
    ...
    switch (action) {
      case "PROJECT_CREATED":
        return <span>{userName} initialized the project workspace.</span>;
      case "HACKER_HIRED":
        return <span>{userName} hired a new operator.</span>;
      case "FINDING_CREATED":
        return <span>{userName} reported a new vulnerability finding.</span>;
      case "PROJECT_KICKOFF":
        return <span>{userName} completed the kickoff checklist.</span>;
      default:
        return (
          <span>
            {userName} performed: {action.toLowerCase().replace(/_/g, " ")}.
          </span>
        );
    }
  };
  ...
  if (loading)
    return (
      <div className="text-xs font-mono text-gray-500 animate-pulse p-4">
  ...
  return (
    <div className="space-y-4 p-4">
      {logs.length === 0 ? (
        <div className="text-xs font-mono text-gray-500 italic p-2">
          No activity or findings recorded for this sector.
        </div>
      ) : (
