...
export default function ConversationItem({
  ...
  const name = isGroup
    ? conv.name
    : other?.fullName || other?.handle || "Unknown";
  // presenceMap is real-time; fall back to the presence stored on the user object from DB
  const online =
    !isGroup &&
    (presenceMap[other?.id]?.isOnline ?? other?.presence?.isOnline ?? false);
  const unread =
    conv.participants?.find((p) => p.userId === myId)?.unreadCount || 0;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
          <span
            className={`font-semibold text-sm truncate ${isActive ? "text-[#00c477]" : "text-white"}`}
          >
            {name}
          </span>
        ...
        <div className="flex items-center justify-between gap-1">
          <span
            className={`text-[9px] font-mono font-black uppercase tracking-wider px-1.5 py-px rounded shrink-0
            ${
              isGroup
