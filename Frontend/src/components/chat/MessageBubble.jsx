...
export default function MessageBubble({ message, isOwn, onReply, onEdit, onDelete }) {
  ...
  return (
    ...
    >
      {!isOwn && <Avatar user={message.sender} size={32} />}
      ...
      <div className={`flex flex-col max-w-[68%] ${isOwn ? 'items-end' : 'items-start'}`}>
        {!isOwn && (
          <span className="text-[11px] font-semibold mb-1 ml-1 font-mono" style={{ color: avatarBg(message.sender?.id) }}>
            {message.sender?.fullName || message.sender?.handle}
          </span>
        )}

        {/* Reply preview */}
        {message.replyTo && !deleted && (
          <div className="mb-1.5 border-l-2 border-[#00c477]/50 bg-white/[0.04] rounded-lg px-3 py-1.5 max-w-full">
        ...
        <div className="flex items-end gap-1.5">
          ...
          <AnimatePresence>
            {hover && !deleted && (
              ...
              >
                <button onClick={() => onReply(message)} className="w-6 h-6 rounded-lg bg-white/[0.06] hover:bg-white/[0.12] text-gray-500 hover:text-gray-300 flex items-center justify-center transition-all" title="Reply">
                ...
                {isOwn && (<>
                  <button onClick={() => onEdit(message)} className="w-6 h-6 rounded-lg bg-white/[0.06] hover:bg-white/[0.12] text-gray-500 hover:text-gray-300 flex items-center justify-center transition-all" title="Edit">
                  ...
                  <button onClick={() => onDelete(message.id)} className="w-6 h-6 rounded-lg bg-white/[0.06] hover:bg-red-500/20 text-gray-500 hover:text-red-400 flex items-center justify-center transition-all" title="Delete">
          ...
          <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words
            ${isOwn ? 'bg-[#00c477] text-[#041a0f] rounded-br-[6px]' : 'bg-[#1c1c1c] text-gray-100 rounded-bl-[6px] border border-white/[0.06]'}
            ...
            {deleted ? (<span className="text-xs">Message deleted</span>) : (<>
              ...
              {isVoice && message.fileUrl && (
                <div className={`flex items-center gap-2 mb-1 px-2 py-1.5 rounded-xl ${isOwn ? 'bg-black/10' : 'bg-black/20'}`}>
              ...
              {message.fileUrl && !isImage && !isVideo && !isVoice && (
                <a href={message.fileUrl} target="_blank" rel="noreferrer"
                  className={`flex items-center gap-2.5 mb-2 px-3 py-2 rounded-xl transition-colors ${isOwn ? 'bg-black/10 hover:bg-black/20' : 'bg-black/20 hover:bg-black/30'}`}>
            ...
            <div className={`flex items-center gap-1 mt-1 ${isOwn ? 'justify-end' : 'justify-start'}`}>
              {message.isEdited && !deleted && <span className={`text-[9px] ${isOwn ? 'text-[#004d2e]' : 'text-gray-600'}`}>edited</span>}
              <span className={`text-[10px] ${isOwn ? 'text-[#004d2e]' : 'text-gray-600'}`}>{fmtTime(message.createdAt)}</span>
              {isOwn && !deleted && (
                <svg className="w-3.5 h-3.5 text-[#004d2e]" viewBox="0 0 16 11" fill="currentColor">
