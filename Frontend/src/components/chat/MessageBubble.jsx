...
export default function MessageBubble({
  ...
  const isImage =
    message.type === "IMAGE" ||
    (message.fileUrl && isImageMime(message.fileMime));
  const isVideo =
    message.type === "VIDEO" ||
    (message.fileUrl && isVideoMime(message.fileMime));
  const senderName = message.sender?.fullName || message.sender?.handle;
  const replySenderName =
    message.replyTo?.sender?.fullName || message.replyTo?.sender?.handle;
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-col max-w-[74%] sm:max-w-[66%] ${isOwn ? "items-end text-right" : "items-start text-left"}`}
      >
        {!isOwn && groupStart && senderName && (
          <span
            className="text-[11px] font-semibold mb-1 ml-1 font-mono"
            style={{ color: avatarBg(message.sender?.id) }}
          >
            {senderName}
          </span>
        )}
        ...
        {message.replyTo && !deleted && (
          ...
          >
            <p
              ...
              style={{ color: avatarBg(message.replyTo.sender?.id) }}
            >
              {replySenderName}
            </p>
            <p
              className={`text-xs truncate ${isOwn ? "text-[#dcecff]/75" : "text-gray-400"}`}
            >
            ...
            {hover && !deleted && (
              ...
              >
                <button
                  ...
                  className="w-6 h-6 rounded-lg bg-white/10 hover:bg-white/20 text-gray-500 hover:text-gray-300 flex items-center justify-center transition-all"
                  title="Reply"
                >
                  <svg
                    className="w-3 h-3"
                ...
                {isOwn && (
                  <>
                    ...
                    <button
                      ...
                      className="w-6 h-6 rounded-lg bg-white/10 hover:bg-red-500/20 text-gray-500 hover:text-red-400 flex items-center justify-center transition-all"
                      title="Delete"
                    >
                      <svg
                        className="w-3 h-3"
          ...
          >
            {deleted ? (
              ...
              <>
                ...
                {isVideo && message.fileUrl && (
                  <video
                    controls
                    className="max-w-[280px] max-h-[200px] rounded-xl mb-2"
                    src={message.fileUrl}
                  />
                )}
                ...
                {isVoice && message.fileUrl && (
                  <div
                    className={`flex items-center gap-2 mb-1 px-2 py-1.5 rounded-xl ${isOwn ? "bg-black/10 justify-end" : "bg-black/20 justify-start"}`}
                  >
                    <svg
                ...
                {message.fileUrl && !isImage && !isVideo && !isVoice && (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {message.fileSize && (
                        <p
                          className={`text-[10px] ${isOwn ? "text-[#004d2e]" : "text-gray-500"}`}
                        >
            ...
            >
              {message.isEdited && !deleted && (
                <span
                  className={`text-[9px] ${isOwn ? "text-[#dcecff]/75" : "text-gray-600"}`}
                >
                  edited
                </span>
              )}
              <span
                className={`text-[10px] ${isOwn ? "text-[#dcecff]/75" : "text-gray-500"}`}
              >
                {fmtTime(message.createdAt)}
              </span>
              {isOwn && !deleted && (
                <svg
                  className="w-3.5 h-3.5 text-[#dcecff]/80"
                  viewBox="0 0 16 11"
                  fill="currentColor"
                >
