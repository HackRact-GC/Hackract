...
export default function ChatSidebar({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setFindLoading(true);
      try {
        setFindResults(await chatApi.searchUsers(findQuery, targetRole));
      } catch {
        setFindResults([]);
      } finally {
  ...
  const filtered = conversations.filter((c) => {
    ...
    const name =
      c.type === "GROUP" ? c.name : other?.fullName || other?.handle || "";
  ...
  return (
    <>
      {/* Main sidebar */}
      <div
        className={`flex flex-col border-r border-white/[0.06] bg-[#111] transition-all duration-300 ${showFind ? "w-0 overflow-hidden" : "w-[320px] shrink-0"}`}
      >
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${connected ? "bg-[#00c477]" : "bg-gray-600"}`}
                />
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingConvs ? (
            ...
            <div className="flex flex-col items-center justify-center h-52 gap-4 px-6 text-center">
              ...
              <button
                onClick={() => setShowFind(true)}
                className="text-[#00c477] text-xs font-semibold hover:underline"
              >
                ...
                <>
                  {findResults.some((u) => u.presence?.isOnline) && (
                    <p className="text-[9px] font-mono font-black uppercase tracking-widest text-[#00c477]/60 px-4 pt-3 pb-1.5">
                      ● Online —{" "}
                      {findResults.filter((u) => u.presence?.isOnline).length}{" "}
                      of {findResults.length}
                    </p>
                  )}
                  {findResults.map((u) => {
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            ...
                            {online && (
                              <span className="w-1.5 h-1.5 rounded-full bg-[#00c477] shrink-0" />
                            )}
                          </div>
                          <div className="flex items-center gap-1.5">
                            <span
                              className={`text-[9px] font-mono font-black uppercase tracking-wider px-1.5 py-px rounded
                        ...
                        <span
                          className={`text-[11px] font-semibold shrink-0 ${online ? "text-[#00c477]" : "text-gray-700"}`}
                        >
            ...
            <AnimatePresence>
              {inviteTarget && (
                <motion.div
                  ...
                  exit={{ y: 20, opacity: 0 }}
                  className="border-t border-white/[0.06] bg-[#0d0d0d] p-4"
                >
                  <div className="flex items-center gap-2.5 mb-3">
                    <Avatar user={inviteTarget} size={32} />
                    <div>
                      <p className="text-xs font-semibold text-white">
                        {inviteTarget.fullName}
                      </p>
                    ...
                    <button
                      ...
                      className="ml-auto w-6 h-6 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] flex items-center justify-center text-gray-500 hover:text-white transition-colors"
                    >
                      <svg
                        className="w-3 h-3"
                        fill="none"
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/[0.04] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#00c477]/30 resize-none transition-all mb-3"
                  />
                  <button
