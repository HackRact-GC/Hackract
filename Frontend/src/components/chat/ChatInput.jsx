...
export default function ChatInput({
  ...
  const handleSend = async () => {
    ...
    if (attachedFile || audioBlob) {
      ...
      try {
        ...
        import("react-hot-toast").then((t) =>
          t.default.error("Upload failed. Please try again."),
        );
        return;
      }
    }
    ...
    const payload = {
      content:
        content ||
        (audioBlob ? "🎤 Voice message" : attachedFile?.name || "File"),
      type: audioBlob
        ? "VOICE"
        : attachedFile
  ...
  const startRecording = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        import("react-hot-toast").then((t) =>
          t.default.error(
            "Audio recording requires HTTPS or localhost connection.",
          ),
        );
        return;
      }
      ...
      import("react-hot-toast").then((t) =>
        t.default.error("Microphone access denied or not available."),
      );
    }
  };
    ...
    <div className="px-5 py-4 border-t border-white/[0.05] bg-[#0d0d0d] shrink-0">
        ...
        {attachedFile && (
          ...
          >
            ...
            <button
              ...
              className="w-5 h-5 rounded-full bg-white/[0.08] hover:bg-red-500/20 flex items-center justify-center text-gray-500 hover:text-red-400 transition-colors shrink-0"
            >
              <svg
                className="w-2.5 h-2.5"
                fill="none"
          ...
          >
            ...
            <div className="flex items-center gap-1 ml-auto">
              ...
              <button
                ...
                className="ml-2 flex items-center gap-1.5 bg-red-500 hover:bg-red-400 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
              >
                <svg
                  className="w-3 h-3"
                  fill="currentColor"
      ...
      {!isRecording && (
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.05] flex items-center justify-center text-gray-500 hover:text-gray-300 transition-all shrink-0 mb-0.5"
            title="Attach file"
          >
            <svg
              className="w-5 h-5"
          ...
          {canSend ? (
            <motion.button
              onClick={handleSend}
              whileTap={{ scale: 0.92 }}
              className="w-10 h-10 rounded-xl bg-[#00c477] hover:bg-[#00d485] flex items-center justify-center text-[#041a0f] transition-all shadow-[0_0_16px_rgba(0,196,119,0.3)] shrink-0 mb-0.5"
              title="Send"
            >
              <svg
                className="w-5 h-5"
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl bg-white/[0.04] hover:bg-red-500/10 hover:border-red-500/30 border border-white/[0.05] flex items-center justify-center text-gray-500 hover:text-red-400 transition-all shrink-0 mb-0.5"
              title="Record voice"
            >
              <svg
                className="w-4 h-4"
      ...
      {!isRecording && !editing && (
        <p className="text-[10px] text-gray-700 font-mono mt-2 text-center">
          Enter to send · Shift+Enter for new line · 🎤 for voice · 📎 for files
        </p>
      )}
    </div>
  );
